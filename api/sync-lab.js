// Lab Diamond Sync - Type-Specific Endpoint
// Syncs lab-grown diamonds incrementally, respecting rate-limits and request timeouts
...
const TYPE = 'lab';
const PROGRESS_FILE = '/tmp/sync_progress_lab.json';
const BATCH_SIZE = 50;
const PAGES_PER_RUN = 1;

function logInfo(runId, message, extra = null) {
  if (extra !== null) {
    console.log(`[SYNC-LAB][${runId}] ${message}`, extra);
    return;
  }
  console.log(`[SYNC-LAB][${runId}] ${message}`);
}

function logError(runId, message, extra = null) {
  if (extra !== null) {
    console.error(`[SYNC-LAB][${runId}] ${message}`, extra);
    return;
  }
  console.error(`[SYNC-LAB][${runId}] ${message}`);
}
...
async function createProduct(item, accessToken, runId) {
  ...
  const description = buildHtmlDescription(item, FIELD_MAPPINGS.lab);
  ...
  ['Diamond_Type', 'Lab', 'Color', 'Clarity', 'Shape'].forEach(k => {
    const v = item[k];
  ...
  try {
    ...
    await addProductToCollection(productId, 'Lab-Grown Diamonds', accessToken, runId);
...
async function attachCertificate(productId, certificateUrl, accessToken, runId) {
  ...
  try {
    ...
    // Create file via GraphQL fileCreate mutation
    const query = `
...
async function syncLab(runId) {
  logInfo(runId, '========== LAB DIAMOND SYNC START ==========');
...
module.exports = async (req, res) => {
  ...
  try {
    const result = await syncLab(runId);
