...
async function createLabProduct(item, accessToken, runId) {
  try {
    // Build product exactly like test-lab.js does
    ...
    const tags = ['belgiumdia', 'lab', item.Diamond_Type, item.Lab, item.Color, item.Clarity];
    ...
    console.log(`[${runId}] Created lab product: SKU=${item.Stock_No}, ID=${productId}, Images=${imageUrls.length}`);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.log(`[${runId}] Lab sync started - max_create=${maxCreates}`);

    const accessToken = await getAccessToken(runId);

    // Load progress
    const progressFile = '/tmp/sync_progress_lab.json';
    ...
    url.searchParams.append('type', 'lab');
    ...
    for (let i = progress.current_index; i < items.length && created < maxCreates; i++) {
      const item = items[i];
      const result = await createLabProduct(item, accessToken, runId);
