// Jewelry Sync - Uses exact same logic as working test-jewelry.js
...
async function createJewelryProduct(item, accessToken, runId) {
  try {
    // Build product exactly like test-jewelry.js does
    ...
    console.log(`[${runId}] Created jewelry: SKU=${item.item}, ID=${productId}, Images=${imageUrls.length}`);
    ...
    return { id: productId, sku: item.item };
  } catch (error) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.log(`[${runId}] Jewelry sync started - max_create=${maxCreates}`);

    const accessToken = await getAccessToken(runId);

    // Load progress
    const progressFile = '/tmp/sync_progress_jewelry.json';
    ...
    url.searchParams.append('type', 'jewelry');
    ...
    for (let i = progress.current_index; i < items.length && created < maxCreates; i++) {
      const item = items[i];
      const result = await createJewelryProduct(item, accessToken, runId);
