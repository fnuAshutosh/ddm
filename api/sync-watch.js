// Watch Sync - Type-Specific Endpoint
// Syncs watches incrementally, respecting rate-limits and request timeouts
...
const TYPE = 'watch';
const PROGRESS_FILE = '/tmp/sync_progress_watch.json';
const BATCH_SIZE = 50;
const PAGES_PER_RUN = 1;

function logInfo(runId, message, extra = null) {
  if (extra !== null) {
    console.log(`[SYNC-WATCH][${runId}] ${message}`, extra);
    return;
  }
  console.log(`[SYNC-WATCH][${runId}] ${message}`);
}

function logError(runId, message, extra = null) {
  if (extra !== null) {
    console.error(`[SYNC-WATCH][${runId}] ${message}`, extra);
    return;
  }
  console.error(`[SYNC-WATCH][${runId}] ${message}`);
}
